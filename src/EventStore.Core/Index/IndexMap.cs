...
namespace EventStore.Core.Index
{
    public class IndexMap
    {
        ...
        public static IndexMap FromFile(string filename, int maxTablesPerLevel = 4, bool loadPTables = true, int cacheDepth = 16, IndexVerificationType indexVerification = IndexVerificationType.MD5)
        {
            ...
            using (var f = File.OpenRead(filename))
            {
                ...
                using (var reader = new StreamReader(f))
                {
                    ...
                    var tables = loadPTables ? LoadPTables(reader, filename, checkpoints, cacheDepth, indexVerification) : new List<List<PTable>>();
        ...
        private static List<List<PTable>> LoadPTables(StreamReader reader, string indexmapFilename, TFPos checkpoints, int cacheDepth, IndexVerificationType indexVerification)
        {
            ...
            while ((text = reader.ReadLine()) != null)
            {
                ...
                try
                {
                    ...
                    ptable = PTable.FromFile(ptablePath, cacheDepth, indexVerification);
